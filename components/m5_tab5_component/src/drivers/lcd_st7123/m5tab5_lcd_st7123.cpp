...
#include "drivers/lcd_st7123/esp_lcd_st7123.h"
...
static m5tab5_lcd_st7123_handles_t s_handles;
...
esp_err_t m5tab5_lcd_st7123_init(m5tab5_runtime_t* runtime)
{
    ...
    esp_lcd_dsi_bus_config_t bus_cfg = ST7123_PANEL_BUS_DSI_2CH_CONFIG();
    ...
    esp_lcd_dbi_io_config_t dbi_cfg = ST7123_PANEL_IO_DBI_CONFIG();
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_dbi(s_handles.mipi_dsi_bus, &dbi_cfg, &s_handles.io), TAG,
                        "new panel IO failed");

    // 5. Create the ST7123 panel / 创建 ST7123 面板
    esp_lcd_dpi_panel_config_t dpi_cfg = ST7123_720_1280_PANEL_66HZ_DPI_CONFIG(LCD_COLOR_FMT_RGB565);
    ...
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_st7123(s_handles.io, &panel_cfg, &s_handles.panel), TAG,
                        "new st7123 panel failed");
    ...
    ESP_LOGI(TAG, "ST7123 720x1280 MIPI-DSI display initialized");

    runtime->display_handle = &s_handles;
    return ESP_OK;
}

const m5tab5_display_driver_descriptor_t k_m5tab5_lcd_st7123_driver = {
    .id   = "m5tab5.lcd.st7123",
    .init = m5tab5_lcd_st7123_init,
};
...
{
    return &k_m5tab5_lcd_st7123_driver;
}
