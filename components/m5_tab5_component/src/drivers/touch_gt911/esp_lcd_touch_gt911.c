...
/* GT911 support key num / GT911 支持的按键数量 */
                                      ...
                                      esp_lcd_touch_handle_t *out_touch)
{
    ...
    /* Only supported callbacks are set / 仅设置已支持的回调 */
    ...
    esp_lcd_touch_io_gt911_config_t *gt911_config =
        (esp_lcd_touch_io_gt911_config_t *)esp_lcd_touch_gt911->config.driver_data;

    /* Prepare pin for touch controller reset / 配置触摸控制器复位引脚 */
    ...
    if (gt911_config && esp_lcd_touch_gt911->config.rst_gpio_num != GPIO_NUM_NC &&
        ...
        /* Prepare pin for touch controller int / 配置触摸控制器中断引脚 */
        ...
        ESP_RETURN_ON_ERROR(
            gpio_set_level(esp_lcd_touch_gt911->config.rst_gpio_num, esp_lcd_touch_gt911->config.levels.reset), TAG,
        ...
        /* Select I2C addr, set output high or low / 选择 I2C 地址并设置引脚高低电平 */
        uint32_t gpio_level;
        ...
        ESP_RETURN_ON_ERROR(gpio_set_level(esp_lcd_touch_gt911->config.int_gpio_num, gpio_level), TAG,
                            "GPIO set level error!");
    ...
    /* Prepare pin for touch interrupt / 配置触摸中断引脚 */
    if (esp_lcd_touch_gt911->config.int_gpio_num != GPIO_NUM_NC) {
        ...
        /* Register interrupt callback / 注册中断回调 */
        if (esp_lcd_touch_gt911->config.interrupt_callback) {
            esp_lcd_touch_register_interrupt_callback(esp_lcd_touch_gt911,
                                                      esp_lcd_touch_gt911->config.interrupt_callback);
        }
    }

    /* Read status and config info / 读取状态与配置信息 */
...
static esp_err_t esp_lcd_touch_gt911_exit_sleep(esp_lcd_touch_handle_t tp)
{
    ...
    if (esp_lcd_touch_gt911->config.int_gpio_num != GPIO_NUM_NC) {
        /* Step 1: Drive INT HIGH to wake GT911 (OUTPUT push-pull) */
        /* 步骤 1：将 INT 拉高以唤醒 GT911 */
        const gpio_config_t int_gpio_config_high = {.mode         = GPIO_MODE_OUTPUT,
                                                    .pin_bit_mask = BIT64(esp_lcd_touch_gt911->config.int_gpio_num)};
        ...
        /* Step 2: INT SYNC — drive LOW for 25ms, then release to INPUT
         * (Ref: Goodix official gtp_wakeup_sleep → gtp_int_sync(ts, 25))
         * This signals the GT911 to enter normal coordinate-reading mode.
         * The original ESP-IDF driver set OUTPUT_OD here which is incorrect
         * per the Goodix reference driver.
         * 步骤 2：INT 同步 — 拉低 25ms，释放为 INPUT
         * GT911 官方驱动在唤醒后执行 gtp_int_sync(ts, 25)
         */
        gpio_set_level(esp_lcd_touch_gt911->config.int_gpio_num, 0);
        vTaskDelay(pdMS_TO_TICKS(25));

        const gpio_config_t int_gpio_config_input = {
            .mode         = GPIO_MODE_INPUT,
            .intr_type    = (esp_lcd_touch_gt911->config.levels.interrupt ? GPIO_INTR_POSEDGE : GPIO_INTR_NEGEDGE),
            .pin_bit_mask = BIT64(esp_lcd_touch_gt911->config.int_gpio_num)};
        ret = gpio_config(&int_gpio_config_input);
        ESP_RETURN_ON_ERROR(ret, TAG, "Input GPIO config failed");
    }
...
static esp_err_t esp_lcd_touch_gt911_read_data(esp_lcd_touch_handle_t tp)
{
    ...
    /* [DBG] Periodic log of 0x814E and on any touch data */
    {
        static uint32_t s_poll_cnt  = 0;
        static uint32_t s_touch_cnt = 0;
        if ((buf[0] & 0x80) == 0x80) {
            s_touch_cnt++;
            if (s_touch_cnt <= 5 || s_touch_cnt % 100 == 0) {
                ESP_LOGI(TAG, "[DBG] 0x814E=0x%02X touch_cnt=%u total_touch=%lu poll=%lu", buf[0],
                         (unsigned)(buf[0] & 0x0F), s_touch_cnt, s_poll_cnt);
            }
        } else if (s_poll_cnt % 500 == 0) {
            ESP_LOGI(TAG, "[DBG] 0x814E=0x%02X (no data) poll=%lu touch=%lu", buf[0], s_poll_cnt, s_touch_cnt);
        }
        s_poll_cnt++;
    }

    /* Any touch data? / 是否存在触摸数据？ */
    if ((buf[0] & 0x80) == 0x00) {
        touch_gt911_i2c_write(tp, ESP_LCD_TOUCH_GT911_READ_XY_REG, clear);
#if (CONFIG_ESP_LCD_TOUCH_MAX_BUTTONS > 0)
    } else if ((buf[0] & 0x10) == 0x10) {
        /* Read all keys / 读取全部按键状态 */
        uint8_t key_max =
            ((ESP_GT911_TOUCH_MAX_BUTTONS < CONFIG_ESP_LCD_TOUCH_MAX_BUTTONS) ? (ESP_GT911_TOUCH_MAX_BUTTONS)
...
static esp_err_t esp_lcd_touch_gt911_del(esp_lcd_touch_handle_t tp)
{
    ...
    /* Reset GPIO pin settings / 复位 GPIO 引脚配置 */
...
/*******************************************************************************
 * Private API function / 私有 API 函数
 *******************************************************************************/
...
static esp_err_t touch_gt911_read_cfg(esp_lcd_touch_handle_t tp)
{
    ...
    /* [DBG] Read additional config registers for diagnostics:
     *   0x8047: Config_Version
     *   0x8048-0x8049: X_Output_Max (little-endian)
     *   0x804A-0x804B: Y_Output_Max (little-endian)
     *   0x804C: Touch_Number
     *   0x804D: Module_Switch1 (bit0=INT trigger type)
     */
    if (touch_gt911_i2c_read(tp, ESP_LCD_TOUCH_GT911_CONFIG_REG, cfg_buf, 7) == ESP_OK) {
        uint16_t x_max = cfg_buf[1] | (cfg_buf[2] << 8);
        uint16_t y_max = cfg_buf[3] | (cfg_buf[4] << 8);
        ESP_LOGI(TAG, "[DBG] Config: ver=%d x_max=%u y_max=%u touch_num=%u module_sw1=0x%02X", cfg_buf[0], x_max, y_max,
                 cfg_buf[5] & 0x0F, cfg_buf[6]);
    }

    /* Read command register 0x8040 */
    uint8_t cmd_reg = 0;
    if (touch_gt911_i2c_read(tp, ESP_LCD_TOUCH_GT911_ENTER_SLEEP, &cmd_reg, 1) == ESP_OK) {
        ESP_LOGI(TAG, "[DBG] Command reg (0x8040)=0x%02X", cmd_reg);
    }
