...
// CJK font declarations (generated by lv_font_conv)
...
void ui_destroy(void)
{
    ...
    // Stop and delete cursor blink timer
    if (cursor_timer) {
    ...
    for (int r = 0; r < TERM_ROWS_MAX; r++) {
        if (row_canvas_bufs[r]) {
            heap_caps_free(row_canvas_bufs[r]);
            row_canvas_bufs[r] = NULL;
        }
...
{
    ESP_LOGI(TAG, "Rebuilding UI for font size %dx%d", font_w, font_h);

    // Select the appropriate font pointer
    if (font_h <= 16) {
        s_active_font = &lv_font_cjk_16;
    } else {
    ...
    // Destroy existing LVGL objects
    ui_destroy();

    // Clear terminal buffer and reset cursor
    term_clear_all();

    // Recreate LVGL objects with new dimensions
    ui_create();

    ESP_LOGI(TAG, "UI rebuilt: %d cols x %d rows", TERM_COLS, TERM_ROWS);
}
