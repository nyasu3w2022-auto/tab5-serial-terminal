...
設定画面の **Input Mode** で起動時の入力モードを選べます。既定値は **Direct** で、従来のシリアル端末入力を変更しません。`Ctrl+Alt+J` は実行中だけの Direct / Japanese (SKK) 切替であり、接続先へは送信されません。Japaneseモードはローマ字SKKとして動作します。小文字ローマ字で入力した確定かなもTAB5のオーバーレイで保持し、Enter、Space、または次の大文字による変換開始でまとめて送信します。
...
送り仮名付きの語では、語頭と送り仮名の先頭を大文字にします。たとえば **`MiRu` → Space → Enter** は辞書キー`みr`を検索して「見る」を確定し、**`KaKu` → Space → Enter** は「書く」を確定します。送り仮名のある候補を選択した場合、候補語幹と送り仮名を連結した結果だけが送信されます。
