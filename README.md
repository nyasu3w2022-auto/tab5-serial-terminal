...
設定画面の **Input Mode** で起動時の入力モードを選べます。既定値は **Direct** で、従来のシリアル端末入力を変更しません。`Ctrl+Alt+J` は実行中だけの Direct / Japanese (SKK) 切替であり、接続先へは送信されません。Japaneseモードは標準的な**ローマ字SKK**として動作します。確定かなは即時送信し、大文字で始めた見出し語と候補だけをTAB5のオーバーレイに表示します。

| 操作 | TAB5上の動作 | 接続先への送信 |
|:---|:---|:---|
| 小文字ローマ字 | ひらがなへ変換して確定 | 変換済みUTF-8を即時送信 |
...
| Space / Enter / Esc（確定かな状態） | 従来の端末動作 | 従来どおり送信 |

カタカナは、確定かな状態で `q` を押してからローマ字を入力します。たとえば `q` → `katakana` → `q` は「カタカナ」を送信し、ひらがな状態へ戻ります。
