...
小書きかなは、temporary ASCII開始キーの`l`と競合しない**`x`系だけ**を使います。`xa`／`xi`／`xu`／`xe`／`xo`は「ぁぃぅぇぉ」、`xya`／`xyu`／`xyo`は「ゃゅょ」、`xtu`または`xtsu`は「っ」、`xwa`は「ゎ」です。`la`、`li`、`ltu`、`lya`などの`l`系別名はサポートしません。`l`を押した後はtemporary ASCII入力として接続先に直接送るためです。
...
Japanese (SKK)モードでは、読みや略語見出し語に含まれるASCII数字列を、Space時に`#`を使うSKK数値テンプレートへ照合できます。たとえば **`1234` → Space** では、補助辞書の基本テンプレートから`1234`、`１２３４`、`一二三四`、`千二百三十四`、`1,234`を候補として表示します。**`dai12kai` → Space** では「第１２回」など、**`2gatu25niti` → Space** では「２月２５日」などを選べます。`/2m25d` → Spaceのように、略語入力からも同じテンプレート検索を使えます。

| 候補記法 | `1234`の展開 | 用途 |
|:---:|:---:|---|
| `#0` | `1234` | 半角数字をそのまま出力 |
| `#1` | `１２３４` | 全角数字 |
| `#2` | `一二三四` | 桁ごとの漢数字。`0`は`〇` |
| `#3` | `千二百三十四` | 位取り付き漢数字。`0`だけなら`零` |
| `#8` | `1,234` | 3桁ごとのカンマ区切り |

数値列が複数ある見出し語では、候補内の`#0`〜`#8`を左から順に対応させます。たとえば`#がつ#にち /#1月#1日/`は、`2がつ25にち`に対して`２月２５日`を作ります。数値を含む**完全一致**のユーザー辞書・補助辞書・システム辞書エントリがある場合は、`#`テンプレートより先に優先します。

独自テンプレートはSDカードの`/TAB5-SKK/SKKUSER.TXT`へ通常のSKK形式で追加し、Import Mergeできます。例として`ver# /v#0/v#1/`は`/ver12` → Spaceで`v12`と`v１２`を候補にします。`#3`の位取り付き漢数字は最大20桁（京）までで、より長い数字では`#0`、`#1`、`#2`、`#8`だけを表示します。`#4`（数値再検索）、`#5`（大字）、`#9`（将棋表記）、小数・負数は未実装です。

> **Flash寿命:** `#`テンプレートから展開された候補は、入力する数値ごとにユーザー辞書を増やさないよう**候補学習の対象外**です。Deferred／Manual設定でもFlash保存やSDバックアップを発生させません。候補の選択、確定、Esc、Backspaceの操作は通常のSKK変換と同じです。
